...
def choice_mechine(message, token):
    primary_button = FlexButton(
        style="primary", action=MessageAction(label="軋一", text=f"(軋一){message[1:]}")
    )
    ...
    bubble = FlexBubble(
        body=FlexBox(
            layout="vertical", spacing="md", contents=[primary_button, secondary_button]
        )
    )
    return ReplyMessageRequest(
        reply_token=token, messages=[FlexMessage(alt_text="機器選擇", contents=bubble)]
    )
...
def create_date_menu(message, token):
    ...
    def create_button(label, action):
        return FlexButton(style="link", height="sm", action=action)
    ...
    for i in range(0, len(folder_links_date), 10):
        ...
        for link in folder_links_date[i : i + 10]:  # noqa E203
            buttons.append(
                create_button(
                    str(link), MessageAction(label=f"{link}", text=name + link)
                )
            )

        bubble = FlexBubble(
            footer=FlexBox(layout="vertical", spacing="sm", contents=buttons, flex=0)
        )
    ...
    return ReplyMessageRequest(
        reply_token=token,
        messages=[FlexMessage(alt_text="Flex Message", contents=carousel)],
    )
...
def create_time_menu(message, token):
    ...
    folder_links = [
        link.split("/")[0] for link in folder_links if link.startswith(date)
    ]
...
def create_imgs_menu(message, token, client_id):
    ...
    images_list = list(
        {
            filename.split("_")[2] + "_" + filename.split("_")[2]: filename
            for filename in img_names
        }.values()
    )  # latest img every minutes
        ...
        for link in images_list[i : i + 10]:  # noqa E203
            buttons.append(
                create_button(
                    str(link),
                    MessageAction(
                        label=f"{'_'.join(link.split('_')[1:4])}", text=name + link
                    ),
                )
            )
...
def handle_text_message(event, messaging_api):
    ...
    if message:
        try:
                    ...
                    messages=[
                        TemplateMessage(
                            ...
                            template=ButtonsTemplate(
                                thumbnail_image_url=(
                                    "https://doqvf81n9htmm.cloudfront.net/data/crop_article/118966/"
                                    "shutterstock_1122707477.jpg_1140x855.jpg"
                                ),
                                title="鋼筋影像",
                                text="功能選單",
                                actions=[
            ...
            elif message.startswith("!(軋一)影像:") or message.startswith(
                "!(軋二)影像:"
            ):
                time_menu = create_time_menu(message, token)
                handle_result = api_reply_message(messaging_api, time_menu)
            elif message.startswith("!(軋一)搜尋:") or message.startswith(
                "!(軋二)搜尋:"
            ):
